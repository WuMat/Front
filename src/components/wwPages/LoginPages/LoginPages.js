  ...
  CheckLogin = () => {
    ...
    axios
      ...
      .then(response => {
        ...
        this.context({ type: "loginHandler" });
      })
      .catch(err => {
        console.log("error");
        this.setState({
    ...
    return (
      <div className="login">
        <div className="card">
          ...
          <form className="login-inputy">{inputs}</form>
          {this.state.loading ? (
            <Spinner />
          ) : (
            <>
              <button className="btn-login" onClick={this.CheckLogin}>
                Zaloguj sie
              </button>
              <div className="registration">{this.state.messageRes}</div>
              <button className="btn-login" onClick={this.registerHandler}>
                Załóż konto
              </button>
            </>
          )}
